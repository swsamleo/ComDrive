...
class Margin_DQN_Env(Env):
    def __init__(self, env_params, sim_params, network, simulator='traci', perception_system=None, safety_system=None):
        ...
        self.dqn = DQN(load_path="margin_net.pth")
        # self.dqn = DQN()
    ...
    def get_state(self, veh_id=None):
        ...
        if lead_veh_id:
            ...
            return [headway, self_speed, lead_speed]
        else:
            return [headway, self_speed, lead_velocity_threshold]
        # headway = []
        # self_speed = []
        # lead_speed = []
    ...
    def calculate_reward(self):
        ...
        temp_traffic_throughput = self.perception_system.get_traffic_throughput(veh_ids)
        temp_fairness_metric = self.safety_system.get_fairness_metric(lambda_para=1, beta=0.5)
        cost_traffic_throughput = (temp_fairness_metric-self.max_fairness_metric)/(self.max_fairness_metric+k)
        cost_fairness_metric = (temp_traffic_throughput-self.max_traffic_throughput)/(self.max_traffic_throughput+k)
        ...
        self.max_traffic_throughput = max(temp_traffic_throughput, self.max_traffic_throughput)
        self.max_fairness_metric = max(temp_fairness_metric, self.max_fairness_metric)
        return r
    ...
    def step(self, rl_actions):
        ...
        # self.update_perception_safety_system()
        # temp_traffic_throughput = self.perception_system.get_traffic_throughput(veh_ids)
        # temp_fairness_metric = self.safety_system.get_fairness_metric(lambda_para=0.09, beta=0.1)
        # cost_traffic_throughput = (temp_fairness_metric-self.max_fairness_metric)/(self.max_fairness_metric+k)
        # cost_fairness_metric = (temp_traffic_throughput-self.max_traffic_throughput)/(self.max_traffic_throughput+k)
        # r1 = (len(veh_ids) - veh)
        # if r1<0:
