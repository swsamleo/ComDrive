...
class DQN(object):
    def __init__(self, load_path=None):
        ...
        if load_path:
            ...
            self.load_path = "margin_net_only_TTC_Bando.pth"
...
class Margin_DQN_only_TTC_Bando_env(Env):
    def __init__(self, env_params, sim_params, network, simulator='traci', perception_system=None, safety_system=None):
        ...
        self.dqn = DQN(load_path="margin_net_only_TTC_Bando.pth")
        # self.dqn = DQN()
    ...
    def step(self,rl_actions):
        ...
        for i in range(len(veh_ids)):
            ...
            r1_ = self.perception_system.get_data_without_noise("velocity", veh_id)/ \
                  (self.perception_system.get_data_without_noise("distance", veh_id)+k)
            if self.perception_system.get_data_without_noise("distance", veh_id) <= collision_distance_offline:
                r1_ = 0


            r2_ = self.safety_system.get_safety_data("TTC", veh_id)
