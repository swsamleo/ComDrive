...
from flow.controllers.car_following_models import IDMController_with_noise_trainning
trained_controller = IDMController_with_noise_trainning
...
class DQN(object):
    def __init__(self, load_path=None):
        ...
        if load_path:
            ...
            self.load_path = "margin_net_mixed_fairness_IDM.pth"
...
class Margin_DQN_mixed_fairness_IDM_env(Env):
    def __init__(self, env_params, sim_params, network, simulator='traci', perception_system=None, safety_system=None):
        ...
        self.dqn = DQN(load_path="margin_net_mixed_fairness_IDM.pth")
        # self.dqn = DQN()
    ...
    def step(self,rl_actions):
        ...
        for _ in range(self.env_params.sims_per_step):
            ...
            actions = []
            for veh_id in self.k.vehicle.get_ids():
                if isinstance(self.k.vehicle.get_acc_controller(veh_id),trained_controller)==False:
                    continue
        ...
        for i in range(len(veh_ids)):
            ...
            self.dqn.store_transition(preference+cur_state[i-minus_num], actions[i-minus_num], r, preference+veh_state)
