    ...
    def run(self, num_runs, rl_actions=None, convert_to_csv=False):
        ...
        for i in range(num_runs):
            ...
            for j in range(num_steps):
                ...
                # import sqlite3
                # conn = sqlite3.connect('velocity_with_margin.db')
                # c = conn.cursor()
                # velocities = self.env.k.vehicle.get_speed(veh_ids)
                # for index, veh_id in enumerate(veh_ids):
                #     velocity = velocities[index]
                #     c.execute('''Insert into velocity_with_margin values ('%s','%s','%s')''' % (veh_id, velocity, j))
                # conn.commit()
                # conn.close()
                #
                # import sqlite3
                # conn = sqlite3.connect('Safety_metric_with_margin.db')
                # c = conn.cursor()
                # metric = self.env.safety_system.get_fairness_metric(lambda_para=1, beta=0.5)
                # c.execute('''Insert into Safety_metric_with_margin values (%s,%s)'''%(metric,time.time()))
                # conn.commit()
                # conn.close()
                #
                # conn = sqlite3.connect('Throughput_with_margin.db')
                # c = conn.cursor()
                # throughput = self.env.perception_system.get_traffic_throughput(veh_ids)
                # c.execute('''Insert into Throughput_with_margin values (%s,%s)'''%(throughput,time.time()))
                # conn.commit()
                # conn.close
            ...
            import sqlite3
            conn = sqlite3.connect('reward_ring_network_fairness_d1.db')
            c = conn.cursor()
            c.execute('''Insert into reward_ring_network_fairness_d1 
